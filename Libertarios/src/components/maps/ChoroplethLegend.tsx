...
export function ChoroplethLegend({ scale, unit }: ChoroplethLegendProps) {
  ...
  /*
   * Sin tramos —ningún territorio llega al mínimo— la leyenda sigue saliendo,
   * reducida a la muestra de «menos de 5». Antes devolvía `null` y el pie del
   * mapa quedaba como una franja vacía sin explicar por qué todo estaba gris,
   * que es justo la situación del día del lanzamiento.
   */
  return (
    <div className="flex flex-wrap items-end gap-x-5 gap-y-3">
      ...
      <div className={`flex items-center gap-2 ${scale.breaks.length > 0 ? "pb-4" : ""}`}>
        <span

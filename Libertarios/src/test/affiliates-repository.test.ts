...
describe("affiliate repository", () => {
  it("derives Spain's total from its provinces", async () => {
  ...
  it("aggregates every Spain-level figure from its provinces", async () => {
    const spain = (await getCountrySnapshot("ES"))!;
  ...
  it("ranks countries by count, Spain first", async () => {
  ...
  it("makes region shares sum to 1", async () => {
    const spain = (await getCountrySnapshot("ES"))!;
    const total = spain.regions.reduce((a, r) => a + r.share, 0);
    expect(total).toBeCloseTo(1, 6);
  });

  it("returns identical snapshots on repeated calls, so SSR and the client agree", async () => {
    expect(await getEuropeSnapshot()).toEqual(await getEuropeSnapshot());
  });

  it("resolves a country by slug", async () => {
    expect((await getCountrySnapshotBySlug("espana"))?.code).toBe("ES");
    expect(await getCountrySnapshotBySlug("no-existe")).toBeUndefined();
  });

  it("keeps mean positions inside the quadrant's range", async () => {
    for (const c of (await getEuropeSnapshot()).countries) {
...
describe("geo registries", () => {
  it("has unique country codes, slugs and numeric ids", async () => {
  ...
  it("covers all 52 Spanish provinces with unique INE codes", async () => {
    expect(SPAIN_PROVINCES).toHaveLength(52);
    expect(new Set(SPAIN_PROVINCES.map((p) => p.code)).size).toBe(52);
  });

  it("resolves TopoJSON features by numeric id and by name", async () => {
  ...
  it("resolves provinces by INE id and by every bilingual spelling", async () => {
...
describe("formatPerMillion", () => {
  it("never rounds a sparse country down to a bare 0", async () => {
  ...
  it("drops the decimal once it stops carrying information", async () => {
    expect(formatPerMillion(9.4)).toBe("9,4");
    expect(formatPerMillion(315)).toBe("315");
  });

  it("gives every country with affiliates a non-zero reading", async () => {
    for (const c of await getRankedCountries()) {
